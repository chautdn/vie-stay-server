...
const accommodationSchema = new mongoose.Schema({
  ...
  type: {
    ...
    enum: {
      values: ['duplex', 'house', 'apartment_building', 'hotel', 'motel', 'hostel', 'guesthouse', 'resort', 'villa', 'homestay'],
  ...
  documents: [{
    type: {
      ...
      enum: {
        values: ['business_license', 'property_deed', 'tax_certificate', 'fire_safety', 'other'],
  ...
  amenities: [{
    ...
    enum: {
      values: [
        'wifi', 'parking', 'pool', 'gym', 'laundry', 'elevator', 'security',
        'air_conditioning', 'heating', 'kitchen', 'restaurant', 'bar',
        'garden', 'terrace', 'balcony', 'sea_view', 'mountain_view',
  ...
  address: {
    ...
    district: {
      ...
      enum: {
        values: ['Quận Hải Châu', 'Quận Thanh Khê', 'Quận Sơn Trà', 'Quận Ngũ Hành Sơn', 'Quận Liên Chiểu', 'Quận Cẩm Lệ', 'Huyện Hòa Vang', 'Huyện Hoàng Sa'],
...
accommodationSchema.statics.findNearby = function(lat, lng, maxDistance = 2000) {
  return this.find({
    'address.coordinates': {
      $near: {
